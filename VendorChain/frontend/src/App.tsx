...
import { Wallet, History, Ticket, QrCode, LogOut, CheckCircle, CreditCard } from 'lucide-react';
...
function Dashboard() {
  ...
  const handlePayVendor = async () => {
    ...
    try {
      ...
      alert("💸 Payment Sent!");
      refreshData(); // Refresh to show history update if self-paying
    } catch (error: any) {
      console.error(error);
      alert("Error: " + error.message);
    }
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white">
        <h1 className="text-6xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">
          VendorChain
        </h1>
        <button 
          onClick={connectWallet}
          className="bg-blue-600 px-8 py-4 rounded-xl text-xl font-bold hover:bg-blue-500 transition-all shadow-lg shadow-blue-900/50 flex items-center gap-2"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-slate-800/50 backdrop-blur-md p-8 rounded-2xl border border-slate-700 shadow-xl relative overflow-hidden">
            ...
            <button 
              ...
              className="w-full mt-6 border border-slate-600 text-slate-400 py-3 rounded-lg text-sm hover:bg-slate-700 hover:text-white transition-all flex items-center justify-center gap-2"
            >
              <CheckCircle className="w-4 h-4" /> Register as Vendor (One-time)
            </button>
          </div>
